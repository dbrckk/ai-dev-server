...
class ReputationPolicyMigrationCLITests(unittest.TestCase):
    ...
    def target_for(self,workflow_file):
        return {
            ...
            "workflow_path":workflow_file["path"],"workflow_blob_sha":workflow_file["blob_sha"],
            "workflow_sha256":workflow_file["sha256"],"workflow_semantic_digest":workflow_file["semantic_digest"],
            "ci_trust_policy_version":CI_TRUST_POLICY_VERSION,"ci_trust_policy_digest":ci_trust_policy_digest(),
        }
    ...
    def test_review_writes_plan(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue(plan.is_file());self.assertFalse(json.loads(plan.read_text())["authorization_template"]["authorized"])
    ...
    def test_apply_requires_authorized_record(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(cli.main(["apply",str(registry),str(plan),str(auth)]),1)
    ...
    def test_apply_can_auto_collect_github_attestation(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            if reinforced:
                reviews.append({"user":{"login":"bob"},"state":"APPROVED","commit_id":"a"*40,"submitted_at":"2026-01-01T00:00:20Z"});permissions["bob"]="maintain"
            attestation=build(
                plan_value,repository="dbrckk/ai-dev-server",pull_request=7,commit_sha="a"*40,reviews=reviews,permissions=permissions,
                ...
                workflow_file=workflow_file,head_commit_timestamp=1767225600.0,reinforced=reinforced,
            )
            with patch.object(cli,"collect_github_attestation",return_value=attestation),patch.dict("os.environ",{"STUDIO_GITHUB_TOKEN":"token"},clear=False):
    ...
    def test_bind_target_reissues_content_bound_plan(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch.object(cli,"collect_review_target",return_value=target),patch.dict("os.environ",{"STUDIO_GITHUB_TOKEN":"token"},clear=False):
            ...
            self.assertEqual(bound["github_review_target"]["commit_sha"],"a"*40);self.assertFalse(bound["authorization_template"]["authorized"])
