...
def run_registered_stages(request_path: str, project_out: Path, work: str,
                          report: dict, deadline: float, runner, clock=time.monotonic) -> dict:
    seen: set[str] = set()
    while True:
...
def run_project(request_path: str, project_out: Path, work: str, runner,
                deadline: float, clock=time.monotonic) -> dict:
    ...
    return run_registered_stages(
        request_path, project_out, work, load_report(project_out), deadline, runner, clock)
