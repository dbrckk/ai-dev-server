...
class EvolutionDifferentialTests(unittest.TestCase):
    ...
    def test_pinned_baseline_is_required(self):
        with self.assertRaisesRegex(DifferentialRejected,'pinned SHA'): evaluate(self.order(),self.validated(),self.diff_result('c'*40,failures=3,passed=False),self.diff_result('b'*40))
