    ...
    def __call__(self, url, **kwargs):
        ...
        if url == 'https://api.github.com/repos/flutter/packages':
            payload = {'html_url': 'https://github.com/flutter/packages', 'pushed_at': '2026-09-01T12:00:00Z',
                'archived': False, 'license': {'spdx_id': 'BSD-3-Clause'}}
            return FetchResult(url, url, 200, {'content-type': 'application/json'}, json.dumps(payload).encode())
        if url.startswith('https://api.github.com/search/repositories?'):
            payload = {'items': [{'html_url': 'https://github.com/flutter/packages',
                'pushed_at': '2026-09-01T12:00:00Z', 'archived': False,
                'license': {'spdx_id': 'BSD-3-Clause'}, 'stargazers_count': 5000, 'default_branch': 'main'}]}
            return FetchResult(url, url, 200, {'content-type': 'application/json'}, json.dumps(payload).encode())
...
class EvolutionResearchTests(unittest.TestCase):
    def work_order(self):
        return {'candidate_id': 'billing-qa-deadbeef1234', 'research_tasks': [
            {'id': 'research-1', 'kind': 'official_docs', 'query': 'Google Play Billing integration and test purchases', 'requirements': []},
            {'id': 'research-2', 'kind': 'package_registry', 'query': 'Flutter in_app_purchase maintained package and changelog', 'requirements': []},
            {'id': 'research-3', 'kind': 'github_source', 'query': 'Flutter billing reference implementation tests', 'requirements': []},
        ]}
    ...
    def test_v2_evidence_requires_content_hash(self):
        ...
        evidence = {'version': 2, 'candidate_id': 'x', 'items': [{
            'task_id': 'research-1', 'kind': 'official_docs',
            'source': 'https://developer.android.com/resource', 'version_or_revision': '1',
            'license': 'official_documentation_terms', 'maintenance_signal': 'current',
            'risks': ['cross_check_required'], 'notes': 'metadata only', 'content_sha256': 'bad'}]}
