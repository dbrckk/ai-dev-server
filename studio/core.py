...
def request_check(data):
    ...
    if set(data) - (required | {'max_rounds', 'max_calls', 'max_cycles', 'priority', 'play_publish', 'max_project_model_calls', 'max_project_repair_calls', 'max_api_cost_usd', 'production_os'}) or not required <= set(data):
    ...
    if project_calls is not None:
        if type(project_calls) is not int or not 1 <= project_calls <= 500:
            raise StudioError('Invalid max_project_model_calls')
    repair_calls = data.get('max_project_repair_calls')
    if repair_calls is not None:
        if type(repair_calls) is not int or not 0 <= repair_calls <= 200:
...
class Model:
    ...
    def _ask(self, role, context, screenshots=()):
        ...
        if not isinstance(local_weighted_contexts, list):
            local_weighted_contexts = []
        ...
        for provider in provider_candidates:
            ...
            if provider.unmetered and ':' in provider.name:
                ...
                parsed_local_contexts = [
                    ...
                    for item in local_weighted_contexts
