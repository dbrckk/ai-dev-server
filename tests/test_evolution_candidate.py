...
class EvolutionCandidateTests(unittest.TestCase):
    ...
    def test_completion_registry_smoke_and_evolution_judges_are_protected(self):
        for path in (
            'studio/completion.py', 'studio/stage_registry.py', 'studio/smoke.py',
            'studio/evolution_differential.py', 'studio/evolution_isolated_runner.py',
        ):
    ...
    def test_test_count_must_match_benchmark_assertions(self):
        ...
        candidate['files'][2]['content'] = 'import unittest\nclass BillingCandidateTests(unittest.TestCase):\n    def test_only_one(self):\n        self.assertTrue(True)\n'
        with self.assertRaisesRegex(CandidateRejected, 'one concrete test per benchmark assertion'):
            validate_candidate(self.work_order(), self.research(), candidate)

    def test_skipped_tests_are_rejected(self):
        candidate = self.candidate()
        candidate['files'][2]['content'] = 'import unittest\nclass BillingCandidateTests(unittest.TestCase):\n    @unittest.skip("no")\n    def test_one(self):\n        pass\n    def test_two(self):\n        pass\n'
        with self.assertRaisesRegex(CandidateRejected, 'skipped tests'):
            validate_candidate(self.work_order(), self.research(), candidate)
