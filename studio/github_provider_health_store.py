...
def _validate(data):
    ...
    for name, row in data.items():
        ...
        if type(successes) is not int or type(failures) is not int or type(consecutive) is not int:
...
def _ref(github):
    ...
    if not isinstance(refs, list):
        raise ProviderHealthStoreError("memory branch lookup invalid")
    exact = [x for x in refs if isinstance(x, dict) and x.get("ref") == "refs/heads/" + STATE_BRANCH]
    if len(exact) > 1:
        raise ProviderHealthStoreError("memory branch ambiguous")
...
def load(github):
    ...
    if not isinstance(head, str) or len(head) != 40:
        raise ProviderHealthStoreError("memory branch head invalid")
    ...
    if not isinstance(tree, dict) or tree.get("truncated") or not isinstance(tree.get("tree"), list):
        raise ProviderHealthStoreError("memory tree invalid")
    item = next(
        (x for x in tree["tree"] if isinstance(x, dict) and x.get("path") == STATE_PATH and x.get("type") == "blob"),
        None,
    )
    if item is None:
        return {}
    ...
    if not isinstance(blob, dict) or blob.get("encoding") != "base64":
        raise ProviderHealthStoreError("provider health blob invalid")
    try:
        raw = base64.b64decode(blob["content"], validate=False)
        if len(raw) > MAX_BYTES:
            raise ProviderHealthStoreError("provider health too large")
        value = json.loads(raw.decode("utf-8"))
    except (KeyError, ValueError, UnicodeDecodeError, json.JSONDecodeError):
        raise ProviderHealthStoreError("provider health unreadable") from None
...
def save(github, data):
    ...
    if ref is None:
        ...
        if not isinstance(default, str) or not default:
            raise ProviderHealthStoreError("default branch invalid")
    ...
    if not isinstance(parent, str) or len(parent) != 40:
        raise ProviderHealthStoreError("memory branch parent invalid")
    ...
    if not isinstance(base_tree, str) or len(base_tree) != 40:
        raise ProviderHealthStoreError("memory base tree invalid")
    ...
    if not isinstance(tree_sha, str) or len(tree_sha) != 40:
        raise ProviderHealthStoreError("provider health tree creation failed")
    commit = github.call("POST", github.repo + "/git/commits", {
        "message": "Persist provider health memory",
        "tree": tree_sha,
        "parents": [parent],
    })
    commit_sha = commit.get("sha") if isinstance(commit, dict) else None
    if not isinstance(commit_sha, str) or len(commit_sha) != 40:
        raise ProviderHealthStoreError("provider health commit creation failed")
    if ref is None:
