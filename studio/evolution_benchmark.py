...
def _hash_map(value):
    if not isinstance(value, dict) or not value: raise BenchmarkRejected('Protected hash evidence missing')
    out = {}
...
def _test_result(value):
    ...
    if not isinstance(value['passed'], bool): raise BenchmarkRejected('Unit-test pass flag invalid')
    for key in ('count','failures','errors'):
        if not isinstance(value[key], int) or value[key] < 0: raise BenchmarkRejected('Unit-test count invalid')
...
def _capability(value, expected_gap):
    ...
    for key in ('assertions_total','assertions_passed'):
        if not isinstance(value[key], int) or value[key] < 0: raise BenchmarkRejected('Capability benchmark count invalid')
...
def validate_execution_result(value, expected_gap):
    ...
    if not isinstance(value.get('reversible'), bool): raise BenchmarkRejected('Reversibility evidence invalid')
    return result
...
def evaluate(work_order, baseline, candidate, differential=None):
    ...
    if not isinstance(gap, str): raise BenchmarkRejected('Benchmark gap missing')
    ...
    if baseline['commit_sha'] != baseline_sha: raise BenchmarkRejected('Baseline evidence does not match pinned SHA')
    if candidate['commit_sha'] == baseline_sha: raise BenchmarkRejected('Candidate evidence points to baseline instead of candidate')
    blockers = []; gates = {}
    gates['trusted_unit_tests_pass'] = candidate['unit_tests']['passed']
    if not gates['trusted_unit_tests_pass']: blockers.append('trusted_unit_tests_failed')
    ...
    if not non_regressing: blockers.append('regression_suite_regressed')
    gates['real_flutter_smoke_passes'] = candidate['flutter_smoke_passed']
    if not gates['real_flutter_smoke_passes']: blockers.append('real_flutter_smoke_failed')
    ...
    if not protected_equal: blockers.append('protected_factory_contract_changed')
    gates['candidate_is_reversible'] = candidate['reversible']
    if not candidate['reversible']: blockers.append('candidate_not_reversible')
    capability_passed = candidate['capability_benchmark']['passed']
    if not capability_passed: blockers.append('capability_benchmark_failed')
    gates['differential_improvement_proved'] = _differential(differential, work_order.get('candidate_id'), baseline_sha, candidate['commit_sha'])
    if not gates['differential_improvement_proved'] and 'differential_improvement_proved' in (work_order.get('promotion_gates') or []):
        blockers.append('differential_improvement_not_proved')
    ...
    if unknown: raise BenchmarkRejected('Unknown promotion gate: ' + ','.join(unknown))
    for item in [item for item in required if not gates[item]]:
        marker = 'promotion_gate_failed:' + item
        if marker not in blockers: blockers.append(marker)
    accepted = capability_passed and not blockers
    return {'version':1,'status':'promotion_approved' if accepted else 'promotion_rejected','candidate_id':work_order.get('candidate_id'),'baseline_sha':baseline_sha,'candidate_sha':candidate['commit_sha'],'gap':gap,'gates':gates,'capability_benchmark_passed':capability_passed,'blockers':blockers,'promotion_decision':'approve' if accepted else 'reject'}
