"""Merge a durable autonomous evolution only after trusted GitHub CI succeeds."""
...
def attempt(work_order: dict, token: str, repository: str) -> dict:
    pending = check_pending(work_order, token, repository)
    status = pending.get('status')
    if status == 'no_pending_promotion':
    ...
    number = pending.get('pull_request'); head_sha = pending.get('commit_sha'); branch = pending.get('branch')
    if not isinstance(number, int) or not isinstance(head_sha, str) or not isinstance(branch, str):
        raise AutoMergeError('Pending promotion identity incomplete')
    ...
    if pr.get('base', {}).get('ref') != 'main' or pr.get('head', {}).get('ref') != branch or pr.get('head', {}).get('sha') != head_sha:
        raise AutoMergeError('Promotion pull request identity changed')

    files = _request(api + '/pulls/' + str(number) + '/files?per_page=100', token)
    if not isinstance(files, list) or len(files) != len(_allowed_paths(candidate_id, gap)):
        raise AutoMergeError('Promotion pull request file scope changed')
    names = {item.get('filename') for item in files if isinstance(item, dict)}
    if names != _allowed_paths(candidate_id, gap):
        raise AutoMergeError('Promotion pull request contains unapproved paths')
    ...
    by_name = {run.get('name'): run for run in runs if isinstance(run, dict) and isinstance(run.get('name'), str)}
    missing = REQUIRED_CHECKS - set(by_name)
    if missing:
        return {'status': 'awaiting_required_checks', 'missing_checks': sorted(missing), 'pull_request': number}
    for name in REQUIRED_CHECKS:
        run = by_name[name]
        if run.get('status') != 'completed':
            return {'status': 'awaiting_required_checks', 'missing_checks': [], 'pull_request': number}
        if run.get('conclusion') != 'success':
            raise AutoMergeError('Required promotion check failed: ' + name)
    ...
    merged = _request(api + '/pulls/' + str(number) + '/merge', token, 'PUT', {
        'sha': head_sha, 'merge_method': 'merge',
        'commit_title': 'Promote autonomous capability: ' + gap})
    if not isinstance(merged, dict) or merged.get('merged') is not True:
        raise AutoMergeError('GitHub refused autonomous promotion merge')
    return {'status': 'promotion_merged', 'pull_request': number, 'merge_sha': merged.get('sha'),
            'candidate_id': candidate_id, 'gap': gap}
...
def main(argv=None):
    ...
    try:
        ...
        result=attempt(order,os.environ.get('STUDIO_GITHUB_TOKEN',''),os.environ.get('GITHUB_REPOSITORY',''))
