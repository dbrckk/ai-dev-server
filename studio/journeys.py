...
def validate_journeys(value):
    ...
    for journey in value:
        ...
        name = journey['id']
        if not isinstance(name, str) or not re.fullmatch(r'[a-z][a-z0-9_-]{0,39}', name) or name in ids or name == 'initial':
            raise ValueError('Invalid, duplicate or reserved journey id: ' + repr(name))
        ids.add(name)
        steps = journey['steps']
...
Journey ids must be unique lower-case slugs using a-z, 0-9, hyphen or underscore (max 40 characters); initial is reserved.
