...
class ExistingProjectTests(unittest.TestCase):
    def test_plans_existing_godot_project_without_workflow_or_binary_assets(self):
        ...
        self.assertEqual(result['engine'],'godot'); self.assertIn('project.godot',paths); self.assertIn('scripts/player.gd',paths); self.assertIn('README.md',paths); self.assertNotIn('.github/workflows/validate.yml',paths); self.assertNotIn('assets/sprite.png',paths)
    def test_engine_mismatch_and_ambiguous_markers_fail_closed(self):
        with self.assertRaisesRegex(ExistingProjectError,'does not match'): plan({'truncated':False,'tree':[item('project.godot')]},'flutter')
        with self.assertRaisesRegex(ExistingProjectError,'Ambiguous'): plan({'truncated':False,'tree':[item('project.godot'),item('pubspec.yaml')]})
    def test_rejects_symlink_mode_large_file_and_truncated_tree(self):
        with self.assertRaisesRegex(ExistingProjectError,'metadata'): plan({'truncated':False,'tree':[item('project.godot',mode='120000')]})
        with self.assertRaisesRegex(ExistingProjectError,'too large'): plan({'truncated':False,'tree':[item('project.godot',size=700000)]})
        with self.assertRaisesRegex(ExistingProjectError,'truncated'): plan({'truncated':True,'tree':[item('project.godot')]})
    def test_materialize_validates_all_blobs_before_writing(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp)
            with self.assertRaisesRegex(ExistingProjectError,'size mismatch'): materialize(root,import_plan,fetch)
