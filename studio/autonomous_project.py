...
def _state_paths(project_out: Path) -> tuple[Path, Path]:
    root = Path(project_out) / AUTONOMY_DIR
    return root / "goal.json", root / "capabilities.json"


def ensure_project_goal(project_out: Path, goal_id: str, objective: str, *, max_attempts: int = 20):
    goal_path, registry_path = _state_paths(project_out)
...
def run_persistent_project(
    ...
    *,
    ...
    def execute_cycle(_goal_state):
        ...
        return translate_orchestrator_result(result)
