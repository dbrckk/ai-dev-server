...
class EvolutionSynthesisTests(unittest.TestCase):
    ...
    def test_model_cannot_escape_candidate_scope(self):
        ...
        with self.assertRaises(CandidateRejected): synthesize(self.order(),self.research(),api=FakeAPI(candidate),model='test-model')
    def test_research_is_required_before_model_call(self):
        api=FakeAPI(self.candidate())
        with self.assertRaisesRegex(CandidateRejected,'completed trusted research'): synthesize(self.order(),{'status':'blocked'},api=api,model='test-model')
