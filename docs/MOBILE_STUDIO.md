...
4. Le moteur sélectionné dans `control/ci.json` traite le push sur main. CircleCI est actuellement sélectionné : voir [son activation](CIRCLECI.md). Son mode initial par défaut est `jumpy-baseline` (référence Godot sans secrets ; `smoke` reste disponible pour Flutter) ; la génération des briefs nécessite `mode=queue` et le contexte configuré. Avec GitHub Actions sélectionné, le push sur main déclenche le workflow. La planification toutes les 20 minutes reprend les demandes inachevées, dans les limites fixées. La planification est best effort ; ce n'est pas une horloge garantie. Désactiver une demande empêche les futurs cycles, sans interrompre un job déjà lancé (annulable dans Actions).
...
| `STUDIO_CODE_MODEL` | Modèle général par défaut ; permet un modèle dédié au développement et aux tests. |
...
Le routage distingue le code et les tests de la planification. `STUDIO_CODE_MODEL` permet de configurer un modèle spécialisé disponible sur le fournisseur choisi ; sinon le modèle général est utilisé. Les modèles effectivement sollicités figurent dans `models_used` du rapport. Le modèle Qwen auparavant proposé par défaut a répondu HTTP 410 lors de l’essai réel du 8 septembre 2026 et a donc été retiré du choix par défaut. Une erreur fournisseur reste un blocage explicite, jamais une validation de l’application.
