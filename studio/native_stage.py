...
from native_qa import validate_native
...
def advance(request_path: Path, root: Path, out: Path) -> dict:
    ...
    state.setdefault('release_evidence', {})['native_qa'] = evidence
    state.pop('human_action', None)
    if state.get('status') == 'human_action_required':
        state['status'] = 'validated_preview'
    apply_completion(state)
    apply_external_gate(state, 'native_qa', evidence)

    parent = state.get('checkpoint_commit')
    if not parent:
        raise StudioError('Native QA report has no checkpoint commit')
...
def main() -> int:
    ...
    print(canonical({
        ...
        'native_passed': evidence.get('passed') if isinstance(evidence, dict) else None,
        'tested_permissions': evidence.get('tested_permissions') if isinstance(evidence, dict) else None,
    }))
    if state.get('status') == 'human_action_required':
        return 2
