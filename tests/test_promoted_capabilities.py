...
class PromotedCapabilitiesTests(unittest.TestCase):
    ...
    def test_missing_registry_is_empty(self):
        with tempfile.TemporaryDirectory() as td:
            value = load(Path(td) / "missing.json")
            self.assertEqual(value, {"version": 1, "capabilities": {}})

    def test_provider_path_is_deterministic(self):
        self.assertEqual(
            provider_for("improvement.apply.repeated_failure"),
    ...
    def test_promoted_capability_bootstraps_project_registry(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            name = "improvement.apply.repeated_failure"
            path = self.write(root, {name: self.entry(name)})
            registry = sync_into_registry(new_registry(), path)
            self.assertEqual(registry["capabilities"][name]["provider"], provider_for(name))
            self.assertEqual(
                registry["capabilities"][name]["evidence"]["source"],
                "promoted_factory_capability",
            )
    ...
    def test_provider_redirect_is_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            path = self.write(root, {name: entry})
    ...
    def test_project_provider_conflict_fails_closed(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            name = "improvement.verify.repeated_failure"
            path = self.write(root, {name: self.entry(name)})
            registry = register(
                new_registry(), name, "studio.other_provider", {"tests": "passed"}
            )
            with self.assertRaisesRegex(PromotedCapabilityError, "conflicts"):
                sync_into_registry(registry, path)
